...
export class AccountingService {
  ...
  private isFinanceOverride(actor: any): boolean {
    ...
    const roles = [actor.role, ...(Array.isArray(actor.roles) ? actor.roles : [])]
      .map((role: any) => String(typeof role === 'string' ? role : role?.name || role?.code || '').toUpperCase());
  ...
  private async assertWorkflowAssignment(tenantId: string, actor: any, workflowRole: string) {
    if (this.isFinanceOverride(actor) || typeof actor === 'string') return;
    ...
    if ((data || []).length && !(data || []).some((row: any) => row.user_id === this.actorId(actor))) {
      throw new BadRequestException(`You are not assigned as ${workflowRole.replaceAll('_', ' ').toLowerCase()} for this company.`);
    }
  }
  ...
  async setWorkflowRoleAssignment(tenantId: string, actor: any, body: any) {
    ...
    const permitted = ['JOURNAL_PREPARER','JOURNAL_REVIEWER','JOURNAL_APPROVER','JOURNAL_POSTER','PAYMENT_PREPARER','PAYMENT_APPROVER','PAYMENT_POSTER'];
  ...
  async postJournal(tenantId: string, id: string, actor: any) {
    ...
    if (manualOrRecurring && journal.status !== 'APPROVED') {
      throw new BadRequestException('This manual or recurring journal must be reviewed and approved before posting.');
    }
    if (manualOrRecurring && !this.isFinanceOverride(actor)) {
  ...
  async reviewJournal(tenantId: string, actor: any, id: string, body: any = {}) {
    ...
    if (!this.isFinanceOverride(actor) && journal.created_by && journal.created_by === userId) {
  ...
  async approveJournal(tenantId: string, actor: any, id: string, body: any = {}) {
    ...
    if (!this.isFinanceOverride(actor) && [journal.created_by, review?.reviewed_by].filter(Boolean).includes(userId)) {
  ...
  async createBankAccount(tenantId: string, body: any) {
    if (!body.bank_name || !body.account_id) throw new BadRequestException('Bank name and ledger account are required.');
    const { data, error } = await this.supabase.from('accounting_bank_accounts').insert({ tenant_id: tenantId, account_id: body.account_id, bank_name: body.bank_name, account_name: body.account_name || null, account_number_masked: body.account_number_masked || null, ifsc_or_swift: body.ifsc_or_swift || null, currency_code: body.currency_code || 'INR', opening_balance: Number(body.opening_balance || 0) }).select().single();
  ...
  async importBankTransactions(tenantId: string, body: any) {
    ...
    const { data: bank, error: bankError } = await this.supabase.from('accounting_bank_accounts').select('id,is_active').eq('tenant_id', tenantId).eq('id', bankAccountId).maybeSingle();
    ...
    sourceRows.forEach((row: any, index: number) => {
      const transactionDate = normalise(row.transaction_date);
      const direction = normalise(row.direction).toUpperCase();
      ...
      else validRows.push({ tenant_id: tenantId, bank_account_id: bankAccountId, transaction_date: transactionDate, value_date: normalise(row.value_date) || null, reference_number: normalise(row.reference_number) || null, description: normalise(row.description) || null, amount, direction });
    });
    ...
    if (rowsToInsert.length) {
      const { data, error } = await this.supabase.from('accounting_bank_transactions').insert(rowsToInsert).select();
      if (error) throw new BadRequestException(error.message);
      created = data || [];
    }
    return { created_count: created.length, skipped_count: validRows.length - rowsToInsert.length, invalid_count: errors.length, errors, transactions: created };
  }

  async reconcileBankTransaction(tenantId: string, id: string, body: any) {
    ...
    if (status === 'MATCHED') {
      ...
      const { data: transaction, error: transactionError } = await this.supabase.from('accounting_bank_transactions').select('amount, bank:accounting_bank_accounts!inner(account_id)').eq('tenant_id', tenantId).eq('id', id).maybeSingle();
      ...
      const matchingLedgerLine = (journal.lines || []).find((line: any) => line.account_id === bankLedgerId && Math.abs((Number(line.debit || 0) + Number(line.credit || 0)) - Number(transaction.amount || 0)) < 0.005);
      if (!matchingLedgerLine) throw new BadRequestException('The selected journal must contain this bank ledger for the exact bank-statement amount.');
    }
    const { data, error } = await this.supabase.from('accounting_bank_transactions').update({ reconciliation_status: status, matched_journal_id: body.journal_id || null }).eq('tenant_id', tenantId).eq('id', id).select().maybeSingle();
    if (error) throw new BadRequestException(error.message);
  ...
  async createOpeningBalanceBatch(tenantId: string, userId: string, body: any) {
    const lines = Array.isArray(body.lines) ? body.lines : [];
  ...
  async validateOpeningBalanceBatch(tenantId: string, userId: string, id: string) {
    ...
    const { data, error: updateError } = await this.supabase.from('accounting_opening_balance_batches').update({ status: 'VALIDATED', validated_by: userId, updated_at: new Date().toISOString() }).eq('tenant_id', tenantId).eq('id', id).select().single();
  ...
  async postOpeningBalanceBatch(tenantId: string, userId: string, id: string) {
    const { data: batch, error } = await this.supabase.from('accounting_opening_balance_batches').select('*, lines:accounting_opening_balance_lines(*)').eq('tenant_id', tenantId).eq('id', id).maybeSingle();
    if (error || !batch) throw new NotFoundException('Opening-balance batch not found.');
    if (batch.status !== 'VALIDATED') throw new BadRequestException('Only validated opening-balance batches can be posted.');
    if ([batch.prepared_by, batch.validated_by].filter(Boolean).includes(userId)) throw new BadRequestException('Maker-checker control: posting must be done by a third finance user.');
    ...
    const journal = await this.createJournal(tenantId, userId, { journal_date: batch.as_of_date, source_type: 'OPENING_BALANCE', source_id: batch.id, narration: `Opening balance batch ${batch.batch_number}${batch.source_reference ? ` — ${batch.source_reference}` : ''}`, lines });
    const posted = await this.postJournal(tenantId, journal.id, userId);
    const { data, error: updateError } = await this.supabase.from('accounting_opening_balance_batches').update({ status: 'POSTED', posted_by: userId, posted_journal_id: posted.id, updated_at: new Date().toISOString() }).eq('tenant_id', tenantId).eq('id', id).select().single();
